...
public class Delphi_Articles
{
	...
	public string UpdateArticle(string[] aArticle)
	{
		...
		try
		{
            ...
            param[43] = oSql.NewParameter("@Autoprocess", SqlDbType.VarChar, int.MaxValue, ParameterDirection.Input, aArticle[43]);
    ...
    public DataSet getArticleSearch(string sJobName, string sJobTypeID)
    {
        sSql = "";
        DataSet ds = new DataSet();
        SqlParameter[] param = new SqlParameter[2];
        try
        {
            sSql = "[spGetJob_search]";
            param[0] = oSql.NewParameter("@JobName", SqlDbType.VarChar, int.MaxValue, ParameterDirection.Input, sJobName);
