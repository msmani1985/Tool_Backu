...
public class HRMS_CMB
{
    ...
    {
        sConStr = ConfigurationManager.ConnectionStrings["conStrSQLHRMSCMB"].ConnectionString;
    ...
    {
        return GetDataSet("SELECT empid,desc01,desc02,desc04,Counter,Convert(varchar(30),todate,103) as DOB,datediff(year,todate,getdate()) as age,case when OPTIONYN=1 then 'true' else 'false' end as OPTIONYN,case when NOMINEEYN=1 then 'true' else 'false' end as NOMINEEYN FROM Mas_HRDetails where hrtype=5 and empid=(select empid from master_employee where refno='" + id + "') order by counter", "HRDetails", CommandType.Text);
    }
    ...
    public DataSet GetEmployeeDetail(int employee_id)
    {
        try
        {
            ...
            oCmd.CommandText = "spGetCMBEmpDetails";
    ...
    private void ocmdmitTrans(Boolean bocmdmit)
    {
        if (bocmdmit)
        {
            if (oTran != null)
