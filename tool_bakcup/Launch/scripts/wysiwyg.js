...
var WYSIWYG = {
	...
	Settings: function() {
		...
		this.ImagesDir = "images/wysiwyg_images/";
		
		// Popups Directory
		//this.PopupsDir = "popups/";
		this.PopupsDir = "resource/";
		...
		this.NoValidBrowserMessage = "HTML editor does not support your browser.";
	...
	writeDropDown: function(n, id) {
		...
		output += '<span id="elm_' + dropdown.id + '_' + n + '" class="dropdown" style="width: 170px;display:none;">';
		for (var i = 0; i < dropdown.elements.length;i++) {
			if (dropdown.elements[i]) {
				...
				// output
		  		output += '<button type="button" onClick="WYSIWYG.execCommand(\'' + n + '\',\'' + dropdown.command + '\',\'' + value + '\')\;" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut" style="width: 150px;">';
	...
	invertIELineBreakCapability: function(n) {
		...
		if (editor.event.keyCode==13) {
	    	if (!editor.event.shiftKey) {//alert('br');
